...
let
  winmo = pkgs.writeShellScriptBin "winmo" ''
      ...
      /run/current-system/sw/bin/mkdir -p /home/kotori/win/e 
      /run/current-system/sw/bin/mount /dev/disk/by-uuid/84C6DCB6C6DCAA26 /home/${user}/win/e
      else
      /run/current-system/sw/bin/mount /dev/disk/by-uuid/84C6DCB6C6DCAA26 /home/${user}/win/e
    fi
  '';
in
{
  systemd.services.ldmtool = {
    ...
    serviceConfig.ExecStart = "/run/current-system/sw/bin/ldmtool create all";
    serviceConfig.ExecStop = "/run/current-system/sw/bin/ldmtool remove all";
